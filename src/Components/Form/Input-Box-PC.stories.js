...
const md = (type) => {
  ...
  switch (type) {
    ...
    case 'error':
      ...
      break;
    case 'select':
      result = `Select - 드롭다운 선택 필드`;
      break;
    case 'select-open':
      result = `Select Open - 열린 드롭다운 상태`;
      break;
    case 'textarea':
...
const stories = makeStories(SinglePage, md, ['default', 'focus', 'filled', 'disabled', 'error', 'select', 'select-open', 'textarea', 'textarea-filled'], 'both', ['pc', 'input-box']);
...
export const Select = stories["select"];
export const SelectOpen = stories["select-open"];
